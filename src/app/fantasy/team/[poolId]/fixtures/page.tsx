        ...
        <div className="rounded-2xl border border-[#0B3363]/10 dark:border-white/10 divide-y divide-[#0B3363]/5 dark:divide-white/5">
          {gwMatches.map((m) => (
            <div key={m.id} className="flex items-center justify-between px-4 py-3 text-sm">
              <TeamBadge name={teamNames[m.home_team_id]} slug={teamSlugs[m.home_team_id]} logoUrl={teamLogoUrls[m.home_team_id]} size={22} className="w-2/5" />
              ...
              <TeamBadge name={teamNames[m.away_team_id]} slug={teamSlugs[m.away_team_id]} logoUrl={teamLogoUrls[m.away_team_id]} size={22} className="w-2/5 flex-row-reverse text-right" />
            </div>
          ))}
