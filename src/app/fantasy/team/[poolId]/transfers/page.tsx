...
type Settings = { id: string; season_id: string; budget: number; transfer_cost_points: number; max_banked_transfers: number };
type SquadPlayer = {
  playerId: string;
  name: string;
  position: Position;
...
export default function TransfersPage() {
  ...
  async function loadAll() {
    ...
    const { data: settingsRow } = await supabase
      ...
      .select("id,season_id,budget,transfer_cost_points,max_banked_transfers,seasons(label,competitions(name,division_id))")
      .eq("id", poolId)
      .maybeSingle();
    if (!settingsRow) {
      setLoading(false);
    ...
    const { data: team } = await supabase
      .from("fantasy_teams")
      .select("id,free_transfers_available")
    ...
    setTeamId(team.id);
    setFreeTransfers(team.free_transfers_available);

    const { data: squadRows } = await supabase
      .from("fantasy_team_players")
      .select("player_id,is_starting,is_captain,is_vice_captain,bench_order,players(id,full_name,fpl_name,nickname,position)")
  ...
  async function confirmTransfer() {
    if (!teamId || !outgoing || !incoming || wouldExceedBudget) return;
    setSaving(true);
    setError("");
    ...
    if (isFree) {
      const nextFree = Math.max(0, freeTransfers - 1);
      await supabase.from("fantasy_teams").update({ free_transfers_available: nextFree }).eq("id", teamId);
    }
  ...
  return (
    <div className="min-h-screen flex flex-col bg-white dark:bg-[#0B1220] text-[#0B3363] dark:text-white transition-colors">
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 py-8 flex-1 w-full">
        ...
        {deadline && (
          <div className={`text-xs mb-6 ${isLocked ? "text-red-600 font-semibold" : "text-[#0B3363]/50 dark:text-white/50"}`}>
        ...
        <div className="flex gap-3 mb-6 flex-wrap">
          <div className="rounded-xl bg-white border border-[#0B3363]/10 shadow-sm px-4 py-2.5 text-center">
            <div className="text-[10px] font-bold uppercase text-[#0B3363]/40">Free transfers</div>
            <div className="font-display font-bold text-lg text-[#0B3363]">{freeTransfers}</div>
          </div>
          <div className="rounded-xl bg-white border border-[#0B3363]/10 shadow-sm px-4 py-2.5 text-center">
        ...
        {outgoing && incoming && (
          <div className="rounded-2xl border border-[#0B3363]/10 dark:border-white/10 p-4 mb-6">
            ...
            <div className="text-xs text-[#0B3363]/50 dark:text-white/50 mb-3">
              New bank: TSH {budgetAfterSwap.toFixed(1)}m · {isFree ? "Uses 1 free transfer" : `Costs ${cost} points (no free transfers left)`}
            </div>
